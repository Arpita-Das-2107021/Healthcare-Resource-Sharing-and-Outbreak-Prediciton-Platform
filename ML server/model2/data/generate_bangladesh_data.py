...
Generates simulated pharmacy sales, outbreaks, and social media data for the whole country.
"""
...
# ── 1. Pharmacies — Nationwide (8 Divisions) ──────────────────────────────────
REGIONS = [
...
for region, base_lat, base_lon in REGIONS:
    for i in range(30):
        PHARMACIES.append({
            "pharmacy_id": f"PH{ph_id:04d}",
            "name": f"{region} Care Pharmacy {i+1}",
            "upazila": region,
...
for day in range(NUM_DAYS):
    ...
    for ph in PHARMACIES:
        for (med_name, base, _, _) in MEDICINES:
            mult = get_outbreak_strength(ph["upazila"], med_name, day)
            qty = max(0, int(base * mult * random.uniform(0.8, 1.2)))
            if qty > 0:
                sales_rows.append({"date": date, "pharmacy_id": ph["pharmacy_id"], "medicine_name": med_name, "quantity_sold": qty, "upazila": ph["upazila"]})
...
write_csv("pharmacies.csv", ["pharmacy_id","name","upazila","lat","lon"], PHARMACIES)
write_csv("sales.csv", ["date","pharmacy_id","medicine_name","quantity_sold","upazila"], sales_rows)
...
print(f"Data generation complete: {len(PHARMACIES)} pharmacies, {len(sales_rows):,} sales rows, {len(social_media_rows):,} social media rows.")
