...
the most recent 7 days of pharmacy data.
...
def run_predictions():
    ...
    # 3. Load Two-Factor Social Media Data
    try:
    ...
    # Load pharmacy data (pharmacies.csv or facilities.csv)
    pharmacies = {}
    ph_path = os.path.join(DATA_DIR, "pharmacies.csv")
    fa_path = os.path.join(DATA_DIR, "facilities.csv")
    if os.path.exists(ph_path):
        with open(ph_path) as f:
            for row in csv.DictReader(f): pharmacies[row["pharmacy_id"]] = row
    elif os.path.exists(fa_path):
        with open(fa_path) as f:
            for row in csv.DictReader(f): pharmacies[row["facility_id"]] = row
